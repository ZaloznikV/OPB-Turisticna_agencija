...
def pobrisi_tabelo():
    cur.execute("""
        DROP TABLE atrakcije;
    """)
    conn.commit()
    print("zbrisal sem atrakcije, ups")

def uvozi_podatke():
    with open("podatki/atrakcije.csv", encoding="UTF-8") as f: ## ime ki si ga bomo zbrali
        rd = csv.reader(f)
        ...
        for r in rd:
            ...
            """, r)
            rid, = cur.fetchone()
            print("Uvožena atrakcija %s z ID-jem %d" % (r[0], rid))
    conn.commit()
